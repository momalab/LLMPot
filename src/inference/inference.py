...
class Inference:
    ...
    def __init__(self, finetune_model: FinetunerModel):
        if finetune_model.model_type == "meta-llama":
            ...
            self._model = AutoModelForCausalLM.from_pretrained(f"{finetune_model.output_dir}/{finetune_model.__str__()}", device_map="cpu")
        else:
            self._tokenizer = ByT5Tokenizer.from_pretrained(finetune_model.base_model_id())
            self._model = T5ForConditionalGeneration.from_pretrained(f"{finetune_model.output_dir}/epoch-{finetune_model.epoch}", device_map="cpu")

        self._model.eval()

    def inference(self, input_str: str) -> str:
...
def main():
    ...
    finetune_model = FinetunerModel(model_type=args.mt, model_name=args.mn, dataset_filename=args.csv,
                                    max_epochs=args.max_e, epoch=args.e, precision=args.p, workers=args.w,
