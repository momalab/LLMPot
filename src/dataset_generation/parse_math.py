...
def parse_with_file(protocol: str, capture_layer: str, port: int, pcap: str, csv: str, context_length: int):
    ...
    __parse(protocol, port, cap, csv, context_length)


def parse_without_file(protocol: str, port: int, cap, csv_filename: str, context_length: int):
    __parse(protocol, port, cap, csv_filename, context_length)


def __parse(protocol: str, port: int, cap, csv_filename: str, context_length: int):
    ...
    if context_length > 0:
        with open(f"{DATASET_PARSED}/{csv_filename}.csv", "w") as csv_context:
...
def main():
    ...
    parse_with_file(protocol, capture_layer, port, pcap, csv, context_length)
