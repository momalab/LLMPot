...
class P3Server(S7Comm):
    ...
    def _update_control_logic(self):
        while True:
            ...
            self._DBdata_0[0] = int(str(new_flow_rate_a).split('.')[0])
            self._DBdata_1[0] = int(str('{0:.2f}'.format(new_flow_rate_a)).split('.')[1])
            self._DBdata_2[0] = int(str(new_flow_rate_b).split('.')[0])
            self._DBdata_3[0] = int(str('{0:.2f}'.format(new_flow_rate_b)).split('.')[1])
