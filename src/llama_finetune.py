...
def print_trainable_parameters(model):
    ...
    log.info(f"trainable params: {trainable_params} || all params: {all_param} || trainable%: {100 * trainable_params / all_param}")
...
if __name__ == '__main__':
    ...
    log = logger.setup_custom_logger(f"{model_type}_{model_name}_{csv_filename}_epochs-{epochs}_precision-0"
                                     f"_{datetime.datetime.fromtimestamp(start_time).strftime('%Y%m%dT%H%M')}",
                                     f"{OUTPUTS_DIR}/logs")
