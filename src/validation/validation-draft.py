...
def validate(model: SimpleT5, tokenizer: ByT5Tokenizer, test_set: [], result_file: TextIO, validation_type: str):
    for i in range(1):
        ...
        try:
            if "|" in request:
            ...
            elif "Context :" or "Context ->" in request:
                question = request[request.rindex(" Request 3:") + 1:len(request)] #question= "Request 2:.. Reponse3:"
                print(f"Q: {question}")
                context = request
                ...
                question = request[request.rindex("Request 3:") - 1:request.rindex("Reponse 3:")] #question= "Request 2:.." 
